...
pub const AdapterDetectStage = struct {
    k: u8 = 8,
    ...
    pub fn init(allocator: std.mem.Allocator) !AdapterDetectStage {
        const size = std.math.pow(usize, 4, 8);
        const counts = try allocator.alloc(u64, size);
        @memset(counts, 0);
        return AdapterDetectStage{
            .counts = counts,
            .allocator = allocator,
        };
    }

    pub fn deinit(self: *AdapterDetectStage) void {
