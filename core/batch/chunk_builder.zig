...
pub const ChunkBuilder = struct {
    ...
    pub fn nextChunk(self: *ChunkBuilder) !?[]u8 {
        if (!self.parser.br.is_mmap) {
            if (self.parser.br.end - self.parser.br.pos < self.chunk_size) {
                const read_len = try self.parser.br.fill();
                if (read_len == 0 and self.parser.br.pos >= self.parser.br.end) return null;
            }
        } else {
            if (self.parser.br.pos >= self.parser.br.end) return null;
        }

        const start_pos = self.parser.br.pos;
        ...
        if (end_pos > start_pos) {
            const chunk = self.parser.br.buffer[start_pos..end_pos];
